...
async function fetchBlogs(){
	...
	try{
		...
		container.innerHTML = posts.map(p => {
			const link = p.externalLink || '#';
			const target = p.externalLink ? ' target="_blank" rel="noopener"' : '';
			return `
			<article class="post blog-post">
				...
				<a href="${link}" class="btn"${target}>${p.externalLink ? 'Read on source' : 'Read more'}</a>
			</article>
		`}).join('');
	...
	try {
		...
		members.forEach((member, idx) => {
			...
			const websiteLink = member.website ? `<a href="${member.website}" target="_blank" rel="noopener noreferrer" title="Personal Website">` +
				`<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5h3V9h4v3h3l-5 5z"/></svg></a>` : '';
